...
import { LiveSensor } from "@models";
import { Sensor } from "@prisma/client";
import { prisma } from "@repositories";
import { WQueryString } from "@schemas/in";
import { WSAuthPayload, WsMessage, WsMessageWrap, WsSysInfoPayload } from "@schemas/out";
    ...
    try {
        ...
        if (liveSensor) {
            ...
            try {
                ...
                wlogger.debug(`Receive sys info: id = ${liveSensor.id}, message = ${JSON.stringify(sysInfo)}`);
                ...
                wlogger.error(`Error in sys info request with error = ${err}`);
            }
        }
    } catch (err) {
        wlogger.error(`Setup fail with ip = ${req.ip}, query = ${JSON.stringify(req.query)}, error = ${err}`);
